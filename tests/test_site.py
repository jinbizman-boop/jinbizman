...
class ReactSiteTests(unittest.TestCase):
    ...
    def test_db_has_fifteen_ordered_migrations(self):
        files=sorted((ROOT/'db/migrations').glob('*.sql')); self.assertEqual(len(files),15); self.assertEqual(files[0].name[:3],'001'); self.assertEqual(files[-1].name[:3],'015')
