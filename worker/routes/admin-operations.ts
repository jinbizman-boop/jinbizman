...
import { assertProjectScope, assertServiceScope } from "../lib/authorization";
...
export async function codeGroupsRoute(request: Request, env: Env): Promise<Response> {
  ...
  const rows=await sql`INSERT INTO common_code_groups(group_code,name,description,created_by) VALUES(${code},${name},${text(b?.description,1000)??""},${auth.id}) RETURNING *`;
  await writeAuditLog(request,env,auth,{actionType:"common_code_group.create",targetType:"common_code_group",targetId:Number(rows[0].id),after:rows[0],statusCode:201}); return ok(rows[0],{status:201});
}
...
export async function commonCodeCreateRoute(request: Request, env: Env, groupId:number): Promise<Response> {
  ...
  const sql=getSql(env); const rows=await sql`INSERT INTO common_codes(group_id,code,label,sort_order,is_active) VALUES(${groupId},${code},${label},${Number(b?.sortOrder)||0},${bool(b?.isActive)}) RETURNING *`;
  await writeAuditLog(request,env,auth,{actionType:"common_code.create",targetType:"common_code",targetId:Number(rows[0].id),after:rows[0],metadata:{groupId},statusCode:201}); return ok(rows[0],{status:201});
}
...
export async function approvalTemplatesRoute(request: Request, env: Env): Promise<Response> {
  ...
  const rows=await sql`INSERT INTO approval_templates(template_code,name,document_type,description,requires_project,created_by) VALUES(${code},${name},${doc},${text(b?.description,1000)??""},${bool(b?.requiresProject,false)},${auth.id}) RETURNING *`;
  await writeAuditLog(request,env,auth,{actionType:"approval_template.create",targetType:"approval_template",targetId:Number(rows[0].id),after:rows[0],statusCode:201}); return ok(rows[0],{status:201});
}
...
export async function approvalTemplateStepCreateRoute(request: Request, env: Env, templateId:number): Promise<Response> {
  ...
  const sql=getSql(env); const rows=await sql`INSERT INTO approval_template_steps(template_id,step_order,approver_role_code,approver_user_id,is_required) VALUES(${templateId},${stepOrder},${role},${userId},${bool(b?.isRequired)}) RETURNING *`;
  await writeAuditLog(request,env,auth,{actionType:"approval_template_step.create",targetType:"approval_template_step",targetId:Number(rows[0].id),after:rows[0],metadata:{templateId},statusCode:201}); return ok(rows[0],{status:201});
}
...
export async function projectIssueCreateRoute(request: Request, env: Env): Promise<Response> {
  ...
  const sql=getSql(env); const scope=await assertProjectScope(sql,auth,projectId); if(scope instanceof Response)return scope; const rows=await sql`INSERT INTO project_issues(project_id,wbs_task_id,issue_type,title,description,priority,status,assignee_user_id,reporter_user_id) VALUES(${projectId},${int(b?.wbsTaskId)},${text(b?.issueType,60)??"risk"},${title},${text(b?.description,4000)??""},${text(b?.priority,30)??"medium"},'open',${int(b?.assigneeUserId)},${auth.id}) RETURNING *`;
  await writeAuditLog(request,env,auth,{actionType:"project_issue.create",targetType:"project_issue",targetId:Number(rows[0].id),projectId,after:rows[0],statusCode:201}); return ok(rows[0],{status:201});
}
...
export async function projectMeetingCreateRoute(request: Request, env: Env): Promise<Response> {
  ...
  const sql=getSql(env); const scope=await assertProjectScope(sql,auth,projectId); if(scope instanceof Response)return scope; const rows=await sql`INSERT INTO project_meetings(project_id,title,meeting_at,location,meeting_url,minutes,created_by) VALUES(${projectId},${title},${at}::timestamptz,${text(b?.location,255)??""},${text(b?.meetingUrl,1000)??""},${text(b?.minutes,10000)??""},${auth.id}) RETURNING *`;
  await writeAuditLog(request,env,auth,{actionType:"project_meeting.create",targetType:"project_meeting",targetId:Number(rows[0].id),projectId,after:rows[0],statusCode:201}); return ok(rows[0],{status:201});
}
...
export async function serviceDeploymentsRoute(request: Request, env: Env): Promise<Response> {
  ...
  if(request.method==="GET"){ if(serviceId){const scope=await assertServiceScope(sql,auth,serviceId); if(scope instanceof Response)return scope;} const canGlobal=Array.isArray(auth.roles)&&auth.roles.some((role)=>["super_admin","executive_admin"].includes(role))||hasPermission(auth,"system.update"); const rows=serviceId?await sql`SELECT * FROM service_deployments WHERE service_id=${serviceId} ORDER BY requested_at DESC LIMIT 100`:await sql`SELECT d.* FROM service_deployments d JOIN services s ON s.id=d.service_id WHERE (${canGlobal} OR s.operator_user_id=${auth.id} OR s.tech_owner_user_id=${auth.id} OR (${auth.departmentId ?? null}::bigint IS NOT NULL AND s.owner_department_id=${auth.departmentId ?? null})) ORDER BY d.requested_at DESC LIMIT 100`; return ok({items:rows}); }
  ...
  const scope=await assertServiceScope(sql,auth,sid); if(scope instanceof Response)return scope; const rows=await sql`INSERT INTO service_deployments(service_id,environment,version_label,status,source_ref,notes,requested_by) VALUES(${sid},${text(b?.environment,30)??"production"},${version},'requested',${text(b?.sourceRef,500)??""},${text(b?.notes,2000)??""},${auth.id}) RETURNING *`;
  await writeAuditLog(request,env,auth,{actionType:"service_deployment.request",targetType:"service_deployment",targetId:Number(rows[0].id),serviceId:sid,after:rows[0],statusCode:201}); return ok(rows[0],{status:201});
}
...
export async function serviceDomainCreateRoute(request: Request, env: Env, serviceId:number): Promise<Response> {
  ...
  const sql=getSql(env); const scope=await assertServiceScope(sql,auth,serviceId); if(scope instanceof Response)return scope; const before=(await sql`SELECT * FROM service_domains WHERE service_id=${serviceId} AND locale=${locale} LIMIT 1`)[0]; const rows=await sql`INSERT INTO service_domains(service_id,domain,locale,is_canonical) VALUES(${serviceId},${domain},${locale},${bool(b?.isCanonical,false)}) ON CONFLICT(service_id,locale) DO UPDATE SET domain=EXCLUDED.domain,is_canonical=EXCLUDED.is_canonical,updated_at=now() RETURNING *`;
  await writeAuditLog(request,env,auth,{actionType:"service_domain.upsert",targetType:"service_domain",targetId:Number(rows[0].id),serviceId,before:before??{},after:rows[0]}); return ok(rows[0]);
}

export async function siteBannersRoute(request: Request, env: Env): Promise<Response> {
  const auth=await requirePermission(request,env,request.method==="GET"?"content.read":"content.update"); if(auth instanceof Response)return auth; const sql=getSql(env); if(request.method==="GET"){ const canGlobal=Array.isArray(auth.roles)&&auth.roles.some((role)=>["super_admin","executive_admin"].includes(role))||hasPermission(auth,"system.update"); return ok({items:await sql`SELECT b.* FROM site_banners b JOIN services s ON s.id=b.service_id WHERE (${canGlobal} OR s.operator_user_id=${auth.id} OR s.tech_owner_user_id=${auth.id} OR (${auth.departmentId ?? null}::bigint IS NOT NULL AND s.owner_department_id=${auth.departmentId ?? null})) ORDER BY b.created_at DESC LIMIT 200`}); }
  const b=await readJson(request); const sid=int(b?.serviceId); const code=text(b?.bannerCode,100,true); const title=text(b?.title,255,true); if(!sid||!code||!title)return fail("VALIDATION_ERROR","서비스, 배너 코드, 제목이 필요합니다.",422); const scope=await assertServiceScope(sql,auth,sid); if(scope instanceof Response)return scope; const rows=await sql`INSERT INTO site_banners(service_id,banner_code,locale,title,body,link_url,placement,created_by) VALUES(${sid},${code},${text(b?.locale,10)??"ko"},${title},${text(b?.body,3000)??""},${text(b?.linkUrl,1000)??""},${text(b?.placement,80)??"global"},${auth.id}) RETURNING *`;
  await writeAuditLog(request,env,auth,{actionType:"site_banner.create",targetType:"site_banner",targetId:Number(rows[0].id),serviceId:sid,after:rows[0],statusCode:201}); return ok(rows[0],{status:201});
}

export async function siteNavigationRoute(request: Request, env: Env): Promise<Response> {
  const auth=await requirePermission(request,env,request.method==="GET"?"content.read":"content.update"); if(auth instanceof Response)return auth; const sql=getSql(env); if(request.method==="GET"){ const canGlobal=Array.isArray(auth.roles)&&auth.roles.some((role)=>["super_admin","executive_admin"].includes(role))||hasPermission(auth,"system.update"); return ok({items:await sql`SELECT n.* FROM site_navigation_items n JOIN services s ON s.id=n.service_id WHERE (${canGlobal} OR s.operator_user_id=${auth.id} OR s.tech_owner_user_id=${auth.id} OR (${auth.departmentId ?? null}::bigint IS NOT NULL AND s.owner_department_id=${auth.departmentId ?? null})) ORDER BY n.service_id,n.locale,n.sort_order,n.id`}); }
  const b=await readJson(request); const sid=int(b?.serviceId); const label=text(b?.label,160,true); const href=text(b?.href,1000,true); if(!sid||!label||!href)return fail("VALIDATION_ERROR","서비스, 메뉴명, 링크가 필요합니다.",422); const scope=await assertServiceScope(sql,auth,sid); if(scope instanceof Response)return scope; const rows=await sql`INSERT INTO site_navigation_items(service_id,locale,parent_id,label,href,sort_order) VALUES(${sid},${text(b?.locale,10)??"ko"},${int(b?.parentId)},${label},${href},${Number(b?.sortOrder)||0}) RETURNING *`;
  await writeAuditLog(request,env,auth,{actionType:"site_navigation.create",targetType:"site_navigation_item",targetId:Number(rows[0].id),serviceId:sid,after:rows[0],statusCode:201}); return ok(rows[0],{status:201});
}
...
export async function knowledgeTemplatesRoute(request: Request, env: Env): Promise<Response> {
  ...
  await writeAuditLog(request,env,auth,{actionType:"knowledge_template.create",targetType:"knowledge_template",targetId:Number(rows[0].id),after:rows[0],statusCode:201}); return ok(rows[0],{status:201});
}
