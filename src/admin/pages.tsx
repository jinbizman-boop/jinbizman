...
import { canUseAction, isForbiddenError, isUnauthorizedError, permissionDeniedMessage } from "../lib/permissions";
...
function handleActionError(error: unknown, setNotice: (message: string) => void) {
  if (isUnauthorizedError(error)) {
    navigate("/admin/login", true);
    return;
  }
  setNotice(isForbiddenError(error) ? permissionDeniedMessage : errorMessage(error));
}
...
export function DashboardPage() {
  ...
  useEffect(() => {
    Promise.all([apiFetch("/api/admin/dashboard"), apiFetch("/api/admin/operations-summary")])
      .then(([a,b]) => { setData(asRecord(a)); setOps(asRecord(b)); })
      .catch((error: unknown) => {
        if (isUnauthorizedError(error)) navigate("/admin/login", true);
        else if (isForbiddenError(error)) setForbidden(true);
  ...
  useEffect(() => {
    ...
    apiFetch(endpoint)
      .then(data => {
        ...
        setRows(arr);
        if (!arr.length && data && typeof data === "object" && !Array.isArray(data)) setDetail(asRecord(data));
        setState("ready");
      })
      .catch((error: unknown) => {
        if (isUnauthorizedError(error)) navigate("/admin/login", true);
        else if (isForbiddenError(error)) setState("forbidden");
        else { setError(errorMessage(error)); setState("error"); }
      });
  }, [endpoint]);
...
export function ProjectWbsPage() {
  ...
  const canWrite = canUseAction(user, "projects", "write");
  ...
  useEffect(()=>{ refresh().catch((error: unknown)=> isForbiddenError(error) ? setForbidden(true) : handleActionError(error, setNotice)); },[]);
  useEffect(()=>{ if(projectId) apiFetch(`/api/admin/wbs?projectId=${projectId}`).then(d=>setTasks(toArray(d))).catch((error: unknown)=> isForbiddenError(error) ? setForbidden(true) : handleActionError(error, setNotice)); },[projectId]);
  ...
  return <div className="admin-page"><AdminPageHeader eyebrow="PROJECT OPERATIONS" title="프로젝트 · WBS" description="프로젝트 계획부터 구조화된 업무, 승인 전제, 진척률까지 하나의 실행 흐름으로 관리합니다." />
    ...
    <div className="kanban-grid">{grouped.map(g=><section className="kanban-lane" key={g.status}><div className="lane-title"><StatusBadge value={g.status}/><span>{g.items.length}</span></div>{g.items.map(t=><article className="task-card" key={String(t.id)}><strong>{String(t.title)}</strong><small>진척 {String(t.progress_percent ?? 0)}%</small>{canWrite ? <select value={String(t.status)} onChange={e=>moveTask(Number(t.id),e.target.value)}>{grouped.map(x=><option key={x.status}>{x.status}</option>)}</select> : <StatusBadge value={String(t.status)} />}</article>)}</section>)}</div>
  </div>;
}

export function DailyWorkPage(){
  const user = useAdminUser();
  const canWrite = canUseAction(user, "daily-work", "write");
  const [projects,setProjects]=useState<Array<Record<string,unknown>>>([]),[tasks,setTasks]=useState<Array<Record<string,unknown>>>([]);
  const [projectId,setProjectId]=useState(0);
  const [taskId,setTaskId]=useState(0);
  const [notice,setNotice]=useState("");
  const [forbidden, setForbidden] = useState(false);
  useEffect(()=>{apiFetch("/api/admin/projects").then(d=>{const p=toArray(d);setProjects(p);if(p[0])setProjectId(Number(p[0].id));}).catch((error: unknown)=> isForbiddenError(error) ? setForbidden(true) : handleActionError(error, setNotice));},[]);
  useEffect(()=>{if(projectId)apiFetch(`/api/admin/wbs?projectId=${projectId}`).then(d=>{const t=toArray(d);setTasks(t);if(t[0])setTaskId(Number(t[0].id));}).catch((error: unknown)=> isForbiddenError(error) ? setForbidden(true) : handleActionError(error, setNotice));},[projectId]);
  async function submitReport(e:FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);try{await apiFetch("/api/erp/daily-reports",{method:"POST",body:JSON.stringify({reportDate:today(),projectId,todayFocus:String(f.get("todayFocus")||""),topPriorityText:String(f.get("topPriority")||""),items:[{wbsTaskId:taskId,goalText:String(f.get("goal")),expectedHours:Number(f.get("hours")||0)}]})});setNotice("아침 업무보고가 실제 WBS에 연결되어 제출되었습니다.");}catch(error){handleActionError(error, setNotice);}}
  async function submitLog(e:FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);const progress=Number(f.get("progress")||0),done=f.get("done")==="on";try{await apiFetch("/api/erp/daily-logs",{method:"POST",body:JSON.stringify({logDate:today(),projectId,dailySummary:String(f.get("summary")||""),items:[{wbsTaskId:taskId,workSummary:String(f.get("work")),actualProgress:done?100:progress,isCompleted:done,delayReasonCode:done?"":String(f.get("delay")||"pending"),nextAction:done?"완료":String(f.get("next")||"후속 작업")}]})});setNotice("퇴근 업무일지가 WBS 진척률에 반영되었습니다.");}catch(error){handleActionError(error, setNotice);}}
  ...
  if (forbidden) return <ForbiddenState />;
  return <div className="admin-page"><AdminPageHeader eyebrow="DAILY EXECUTION" title="업무보고 · 업무일지" description="자유 텍스트가 아니라 WBS를 기준으로 계획과 실제를 연결합니다." />{notice&&<div className="notice-line">{notice}</div>}{canWrite ? <div className="admin-split"><form className="admin-panel admin-form" onSubmit={submitReport}><h2>아침 업무보고</h2>{selector}<label>오늘의 초점<input name="todayFocus"/></label><label>최우선 과업<input name="topPriority"/></label><label>오늘 목표<textarea name="goal" required/></label><label>예상 시간<input name="hours" type="number" min="0" max="24" step="0.5" defaultValue="1"/></label><button className="button primary">업무보고 제출</button></form><form className="admin-panel admin-form" onSubmit={submitLog}><h2>퇴근 업무일지</h2>{selector}<label>총평<input name="summary"/></label><label>실제 수행<textarea name="work" required/></label><label>진척률<input name="progress" type="number" min="0" max="100" defaultValue="50"/></label><label className="check-line"><input type="checkbox" name="done"/> 완료</label><label>지연 사유<input name="delay"/></label><label>다음 액션<input name="next"/></label><button className="button primary">업무일지 제출</button></form></div> : <StatePanel kind="empty" title="수행할 수 있는 업무 작성 권한이 없습니다" description="조회 가능한 항목은 표시하되 제출 버튼은 권한이 있는 계정에만 노출됩니다." />}</div>;
}

export function ApprovalPage(){
  const user = useAdminUser();
  const canCreate = canUseAction(user, "approvals", "write");
  const canApprove = canUseAction(user, "approvals", "approve");
  const [rows,setRows]=useState<Array<Record<string,unknown>>>([]),[users,setUsers]=useState<Array<Record<string,unknown>>>([]);
  const [notice,setNotice]=useState("");
  const [forbidden, setForbidden] = useState(false);
  const refresh=async()=>setRows(toArray(await apiFetch("/api/admin/approvals")));
  useEffect(()=>{refresh().catch((error: unknown)=> isForbiddenError(error) ? setForbidden(true) : handleActionError(error, setNotice));apiFetch("/api/admin/users").then(d=>setUsers(toArray(d))).catch(()=>{});},[]);
  async function create(e:FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);try{await apiFetch("/api/erp/approvals",{method:"POST",body:JSON.stringify({documentType:String(f.get("type")||"general"),title:String(f.get("title")),approverUserIds:[Number(f.get("approver"))],status:"submitted",payload:{summary:String(f.get("summary")||"")}})});setNotice("결재가 상신되었습니다.");e.currentTarget.reset();await refresh();}catch(error){handleActionError(error, setNotice);}}
  async function act(id:number,actionType:string){try{await apiFetch(`/api/erp/approvals/${id}/actions`,{method:"POST",body:JSON.stringify({actionType,comment:"React ERP에서 처리"})});setNotice(`결재 ${actionType} 처리가 기록되었습니다.`);await refresh();}catch(error){handleActionError(error, setNotice);}}
  if (forbidden) return <ForbiddenState />;
  return <div className="admin-page"><AdminPageHeader eyebrow="GOVERNANCE" title="전자결재" description="프로젝트·WBS·게시·예산·휴가·평가를 같은 승인 원칙으로 통제합니다." />{notice&&<div className="notice-line">{notice}</div>}<div className="admin-split">{canCreate ? <form className="admin-panel admin-form" onSubmit={create}><h2>새 결재 상신</h2><label>문서 유형<input name="type" defaultValue="general" required/></label><label>제목<input name="title" required/></label><label>승인자<select name="approver" required>{users.map(u=><option key={String(u.id)} value={String(u.id)}>{String(u.name)} · {String(u.department_name||"")}</option>)}</select></label><label>요약<textarea name="summary"/></label><button className="button primary">상신</button></form> : <StatePanel kind="empty" title="결재 상신 권한이 없습니다" description="결재 조회는 가능하지만 새 결재 상신은 권한이 있는 계정에만 노출됩니다." />}<section className="admin-panel"><h2>최근 결재</h2><div className="approval-list">{rows.map(r=><article className="approval-row" key={String(r.id)}><div><StatusBadge value={String(r.status)}/><strong>{String(r.title)}</strong><small>{String(r.document_type)}</small></div>{canApprove ? <div className="approval-actions"><button onClick={()=>act(Number(r.id),"approve")}>승인</button><button onClick={()=>act(Number(r.id),"request_changes")}>보완</button><button onClick={()=>act(Number(r.id),"reject")}>반려</button></div> : null}</article>)}</div></section></div></div>;
}

export function EvaluationPage(){
  const user = useAdminUser();
  const canScore = canUseAction(user, "evaluations", "write");
  const canFinalize = canUseAction(user, "evaluations", "approve");
  const [cycles,setCycles]=useState<Array<Record<string,unknown>>>([]),[users,setUsers]=useState<Array<Record<string,unknown>>>([]),[items,setItems]=useState<Array<Record<string,unknown>>>([]),[evidence,setEvidence]=useState<Array<Record<string,unknown>>>([]);
  const [cycleId,setCycleId]=useState(0),[userId,setUserId]=useState(0),[notice,setNotice]=useState("");
  const [forbidden, setForbidden] = useState(false);
  useEffect(()=>{apiFetch("/api/admin/evaluations").then(d=>{const a=toArray(d);setCycles(a);if(a[0])setCycleId(Number(a[0].id));}).catch((error: unknown)=> isForbiddenError(error) ? setForbidden(true) : handleActionError(error, setNotice));apiFetch("/api/admin/users").then(d=>{const a=toArray(d);setUsers(a);if(a[0])setUserId(Number(a[0].id));}).catch(()=>{});},[]);
  ...
  async function score(e:FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);try{await apiFetch("/api/erp/evaluations/scores",{method:"POST",body:JSON.stringify({cycleId,evaluateeUserId:userId,evaluationItemId:Number(f.get("item")),score:Number(f.get("score")),comment:String(f.get("comment")||"")})});setNotice("평가 근거를 기반으로 점수가 저장되었습니다.");}catch(error){handleActionError(error, setNotice);}}
  async function finalize(){try{await apiFetch(`/api/erp/evaluations/cycles/${cycleId}/finalize`,{method:"POST",body:"{}"});setNotice("평가 주기가 확정되었습니다.");}catch(error){handleActionError(error, setNotice);}}
  if (forbidden) return <ForbiddenState />;
  return <div className="admin-page"><AdminPageHeader eyebrow="EVIDENCE FIRST" title="인사평가" description="점수보다 먼저 WBS·산출물·승인·협업의 근거를 확인합니다." />{notice&&<div className="notice-line">{notice}</div>}<div className="filter-row"><select value={cycleId} onChange={e=>setCycleId(Number(e.target.value))}>{cycles.map(c=><option key={String(c.id)} value={String(c.id)}>{String(c.name)}</option>)}</select><select value={userId} onChange={e=>setUserId(Number(e.target.value))}>{users.map(u=><option key={String(u.id)} value={String(u.id)}>{String(u.name)}</option>)}</select></div><section className="admin-panel"><div className="panel-heading"><div><span className="eyebrow">EVIDENCE</span><h2>평가 근거 {evidence.length}건</h2></div></div>{evidence.length?<DataTable rows={evidence}/>:<StatePanel kind="empty" title="평가 근거가 없습니다" description="근거가 없으면 점수 저장과 평가 확정이 서버에서 차단됩니다."/>}</section><div className="admin-split">{canScore ? <form className="admin-panel admin-form" onSubmit={score}><h2>점수 입력</h2><label>평가 항목<select name="item" required>{items.map(i=><option key={String(i.id)} value={String(i.id)}>{String(i.name)}</option>)}</select></label><label>점수<input name="score" type="number" min="0" max="100" required/></label><label>코멘트<textarea name="comment"/></label><button className="button primary" disabled={!evidence.length}>점수 저장</button></form> : <StatePanel kind="empty" title="평가 점수 입력 권한이 없습니다" description="평가 근거는 조회하되 점수 입력 버튼은 권한 있는 계정에만 노출됩니다." />}<section className="admin-panel finalize-card"><h2>평가 확정</h2><p>근거와 점수가 모두 존재해야 서버에서 확정됩니다.</p>{canFinalize ? <button className="button primary" onClick={finalize} disabled={!cycleId||!evidence.length}>평가 주기 확정</button> : null}</section></div></div>;
}

export function SiteContentPage(){
  const user = useAdminUser();
  const canWrite = canUseAction(user, "site-content", "write");
  const [services,setServices]=useState<Array<Record<string,unknown>>>([]),[rows,setRows]=useState<Array<Record<string,unknown>>>([]);
  const [serviceId,setServiceId]=useState(0),[selectedId,setSelectedId]=useState(0),[notice,setNotice]=useState("");
  const [forbidden, setForbidden] = useState(false);
  ...
  useEffect(()=>{apiFetch("/api/admin/services").then(d=>{const a=toArray(d);setServices(a);if(a[0])setServiceId(Number(a[0].id));}).catch((error: unknown)=> isForbiddenError(error) ? setForbidden(true) : handleActionError(error, setNotice));},[]);
  useEffect(()=>{refresh().catch((error: unknown)=> isForbiddenError(error) ? setForbidden(true) : handleActionError(error, setNotice));},[serviceId]);
  const selected=rows.find(r=>Number(r.id)===selectedId);
  async function transition(status:WorkflowStatus){if(!selected)return;try{await apiFetch(`/api/admin/contents/${selectedId}`,{method:"PATCH",body:JSON.stringify({status})});setNotice(`콘텐츠 상태가 ${status}(으)로 변경되었습니다.`);await refresh();}catch(error){handleActionError(error, setNotice);}}
  if (forbidden) return <ForbiddenState />;
  return <div className="admin-page"><AdminPageHeader eyebrow="CONTENT OPERATIONS" title="홈페이지 운영" description="서비스 단위 콘텐츠 모델, 번역, 발행 상태와 SEO를 실제 데이터로 관리합니다." />{notice&&<div className="notice-line" role="status">{notice}</div>}<div className="filter-row"><label>서비스<select value={serviceId} onChange={e=>setServiceId(Number(e.target.value))}>{services.map(s=><option key={String(s.id)} value={String(s.id)}>{String(s.service_name)}</option>)}</select></label>{rows.length?<label>콘텐츠<select value={selectedId} onChange={e=>setSelectedId(Number(e.target.value))}>{rows.map(r=><option key={String(r.id)} value={String(r.id)}>{String(r.title||r.slug||r.id)}</option>)}</select></label>:null}</div>{rows.length?<><section className="admin-panel"><DataTable rows={rows} caption="홈페이지 콘텐츠 목록"/></section>{selected?<div className="admin-split">{canWrite ? <WorkflowPanel key={`${selectedId}-${String(selected.status)}`} initialStatus={(String(selected.status||"draft") as WorkflowStatus)} onTransition={transition}/> : <StatePanel kind="empty" title="콘텐츠 변경 권한이 없습니다" description="콘텐츠 상태 변경은 권한 있는 계정에만 노출됩니다." />}<RevisionDiff before={JSON.stringify({title:selected.title,slug:selected.slug,status:selected.status},null,2)} after={JSON.stringify({title:selected.title,slug:selected.slug,status:selected.status,review:"ERP workflow-controlled"},null,2)}/></div>:null}</>:<StatePanel kind="empty" title="등록된 콘텐츠가 없습니다" description="정적 샘플을 만들지 않고 실제 CMS 데이터가 생길 때까지 빈 상태를 유지합니다."/>}</div>;
}

export function MediaPage(){
  const user = useAdminUser();
  const canWrite = canUseAction(user, "media", "write");
  const [notice,setNotice]=useState("");
  async function upload(e:FormEvent<HTMLFormElement>){e.preventDefault();const f=new FormData(e.currentTarget);try{const response=await fetch("/api/admin/media",{method:"POST",credentials:"include",body:f});const json=await response.json() as {success:boolean;data?:unknown;error?:{message?:string;code?:string}};if(response.status===401){navigate("/admin/login", true);return;}if(response.status===403)throw new Error(permissionDeniedMessage);if(!response.ok||!json.success)throw new Error(json.error?.message||"업로드 실패");setNotice("R2와 attachments DB에 파일이 저장되었습니다.");}catch(error){setNotice(errorMessage(error));}}
  return <div className="admin-page"><AdminPageHeader eyebrow="R2 MEDIA" title="미디어 라이브러리" description="R2 바인딩이 없으면 성공한 척하지 않고 서버가 명확히 실패합니다." />{notice&&<div className="notice-line">{notice}</div>}{canWrite ? <form className="admin-panel admin-form narrow-form" onSubmit={upload}><label>파일<input type="file" name="file" accept="image/*,.pdf" required/></label><label>서비스 ID<input name="serviceId" type="number" min="1"/></label><label>설명<input name="altText"/></label><button className="button primary">실제 파일 업로드</button></form> : <ForbiddenState description="현재 계정으로는 미디어 업로드 기능에 접근할 수 없습니다." />}</div>;
}
