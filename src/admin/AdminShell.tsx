...
import { canAccessModule, isForbiddenError, isUnauthorizedError } from "../lib/permissions";
...
export function AdminShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    void getCurrentUser()
      .then(setUser)
      .catch((error: unknown) => {
        if (isUnauthorizedError(error) || (error && typeof error === "object" && "status" in error && error.status === 404)) {
          navigate("/admin/login", true);
          return;
        }
        if (isForbiddenError(error)) {
          setAuthError("현재 계정으로는 관리자 영역에 접근할 수 없습니다.");
          return;
        }
  ...
  useEffect(() => {
    ...
    document.addEventListener("keydown", focusFirstNav);
    return () => document.removeEventListener("keydown", focusFirstNav);
  }, []);

  const visibleModules = useMemo(
    () => user ? adminModules.filter((module) => canAccessModule(user, module.key)) : [],
    [user],
  );
  const groups = useMemo(
    () => adminGroups
      .map((group) => [group, visibleModules.filter((module) => module.group === group)] as const)
      .filter(([, modules]) => modules.length),
    [visibleModules],
  );
  const activeModule = adminModules.find((module) => route.pathname === `/admin/${module.key}`)
    ?? adminModules.find((module) => module.key === "dashboard");
  const routeForbidden = Boolean(user && activeModule && !canAccessModule(user, activeModule.key));

  if (loading) return <div className="admin-loading"><StatePanel kind="loading" title="ERP 세션을 확인하고 있습니다."/></div>;
  if (authError) return <div className="admin-loading"><StatePanel kind="error" title="세션을 확인하지 못했습니다." description={authError}/></div>;
  if (!user) return null;
  ...
  return <AdminUserProvider user={user}><div className="admin-root">
    ...
    <div className="admin-main">
      ...
      <main id="admin-main" className="admin-content" tabIndex={-1}>{routeForbidden ? <ForbiddenState /> : children}</main>
    </div>
    {menu ? <button className="admin-sidebar-backdrop" onClick={() => setMenu(false)} aria-label="메뉴 닫기"/> : null}
  </div></AdminUserProvider>;
}
